...
def param_init_decoder_modify(beta, options, params, prefix='decoder_gru'):
# changed    
    n_x = options['n_x']
    n_h = options['n_h']
...
def decoder_layer_modify(tparams, state_below, prefix='decoder_gru'):
# changed    
    ...
    #tparams_W_raw = sample(tparams[_p(prefix, 'W')][0], tparams[_p(prefix, 'W')][1])
    #tparams_W = theano.printing.Print('tparams_W: ')(tparams_W_raw)
    ...
    def _step_slice(x_, xx_, h_, U, Ux):
        #Ux = theano.printing.Print('Ux: ')(theano.tensor.sum(theano.tensor.isnan(Ux)))
        #h_ = theano.printing.Print('h_: ')(h_)
        ...
        preactx = preactx * r
        preactx = preactx + xx_
        #preactx_print = theano.printing.Print('preactx: ')(preactx)
        
        h = tensor.tanh(preact)
        
        h = u * h_ + (1. - u) * h
        #h = theano.printing.Print('h: ')(h_raw)
    ...
    rval, updates = theano.scan(_step,
                                ...
                                n_steps=n_steps-1)
    '''
    rval_raw, updates = theano.scan(_step,
                                sequences=seqs,
                                outputs_info = [h0vec],
                                ...
                                name=_p(prefix, '_layers'),
                                n_steps=n_steps-1)
    rval = theano.printing.Print('rval: ')(rval_raw)
