...
def Santa_modify(beta, batchsize, tparams, cost, inps, lr, eidx, nframes, max_epoch, rho=0.95, anne_rate=0.5, e=1e-8, clip_norm=5):
    ...
    for w, g in zip(tparams.values(), gshared):
        ...
        def sample(w2, r2):
            ...
            return tensor.transpose(r2) * sig2 + mu2 # shape = (batchsize, len(w))
        def logq(wbar1, z1):
            ...
            return tensor.mean(tensor.sum(-((z1 - mu1)/sig1) ** 2 / 2 - .5 * tensor.log(2*np.pi) - logsig1, axis=1))
        def ddw_Lhat_wo_Ez_wbar(wbar):                
            #NOTE: unlike Domke - we put batchsize first in shape of r
            r = trng.normal([batchsize, w.get_value().shape[0]], avg = 0.0, std = 1.0, dtype=theano.config.floatX)
            z = sample(w, r)
